...
export class LoginComponent implements OnInit {
  ...
  userLogin(user:any) {
    ...
    this.http.post("/api/facultyLogin", params, {headers:reqHeader, observe: "response"}).subscribe((res:any) => {
      //console.log(res)
      if (res.body) {
        //console.log(res.body);
        this.person = res.body;
        
        this.storage.set('username', this.person.username);
        this.storage.set('password', this.person.password);
